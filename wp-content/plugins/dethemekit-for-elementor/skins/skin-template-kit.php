...
class dtk_Skin_Posts_Template_Kit extends Skin_Base {
...
//	private $template_cache=[];
  ...
  public $grid_settings=[
	...
	public function get_id() {
		return 'de_loop_posts_template_kit';
	}
  ...
  private function admin_bar_menu(){
    foreach($this->used_templates as $post_id){
      \dtk_Admin_Bar_Menu::add_document($post_id);
    }
  }
    ...
    do_action( 'dtk_after_pagination_controls', $this );
  }
  public function register_navigation_design_controls() {
    do_action( 'dtk_after_style_controls', $this );
  }
    ...
    $this->add_control(
        ...
        'condition' => [
					$this->get_id().'_use_de_grid' => 'yes'
				],
			]
		);
    
    do_action( 'dtk_after_control', $this );
  ...
  $this->add_control(
	...
	public function get_skin_template(){
        ...
        //   WHERE  $wpdb->posts.post_status='publish' AND $wpdb->posts.post_type='envato_tk_templates'"
		// 		);

				// $templates = $wpdb->get_results( 
				// 	"SELECT $wpdb->posts.ID as ID, $wpdb->posts.post_title as post_title FROM $wpdb->posts
          		// 	WHERE  $wpdb->posts.post_status='publish' AND $wpdb->posts.post_type='elementor_library'"
				// );
				...
				$templates = $wpdb->get_results( 
					"SELECT $wpdb->term_relationships.object_id as ID, $wpdb->posts.post_title as post_title FROM $wpdb->term_relationships
						...
						INNER JOIN $wpdb->postmeta ON
							$wpdb->postmeta.post_id=$wpdb->posts.ID AND $wpdb->postmeta.meta_key='_elementor_template_type' AND NOT $wpdb->postmeta.meta_value='kit' 
          WHERE  $wpdb->posts.post_status='publish' AND $wpdb->term_taxonomy.taxonomy='elementor_library_type' AND $wpdb->terms.slug='section'"
				);
  ...
  public function end_grid(){
    ...
    global $dtk_render_loop, $wp_query,$dtk_index;
    $dtk_index++;
    $old_query=$wp_query;
    ...
    $template = apply_filters( 'dtk_action_template', $template,$this,$dtk_index );
    $template = $this->get_current_ID($template);
    
    $dtk_render_loop=get_the_ID().",".$template;
  ...
  protected function render_post_footer() {
   ...
   ?>
	...
	protected function render_post() {
	  do_action( 'dtk_before_render_post_header', $this );
    $this->render_post_header();
    do_action( 'dtk_after_render_post_header', $this );
    ...
    do_action( 'dtk_after_render_post_footer', $this );
    ...
    protected function ajax_pagination(){
      ...
      if($parent_settings[$this->get_id().'_post_slider'] == "yes") {
        ...
        if($parent_settings[$this->get_id().'_use_de_grid'] == "yes" && $parent_settings[$this->get_id().'_de_grid'] ){
          $this->set_custom_grid($parent_settings[$this->get_id().'_de_grid']);
  ...
  protected function render_loadmore_button() {
    ...
    $next_page_link = trailingslashit( get_permalink() ) . '?page='.$next_page;
    $class='';
    $args = [ 'loading_text'  =>  $settings['de_loadmore_loading_text'],
         'text' => $settings['de_loadmore_text'],//or infinitescroll for pro
         'widget_id' => $this->parent->get_id(),
    ...
    $data=htmlentities(json_encode($args), ENT_QUOTES);
		...
		if ( $settings['de_loadmore_hover_animation'] ) {
			$class = 'elementor-animation-' . $settings['de_loadmore_hover_animation'];
		}
  ...
  private function slider_elements(){
    ...
    do_action( 'dtk_after_slider_elements', $this );
  }
...
class dtk_Skin_Archive_Template_Kit extends dtk_Skin_Posts_Template_Kit {
