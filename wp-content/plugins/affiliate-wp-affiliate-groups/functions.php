...
// Get all affiliates from a group
function get_group_affiliates($group_id) {

  $all_affiliates = affiliate_wp()->affiliates->get_affiliates(array('number' => 0));

  $group_affiliates = array();
  ...
  if ( !empty($all_affiliates) ) :
    ...
    foreach ($all_affiliates as $a) :
      ...
      $group = get_affiliate_group($affiliate_id, 'id');

      if($group == $group_id) :

        array_push($group_affiliates, $affiliate_id);
  ...
  return $group_affiliates;

}

/*add_shortcode('affwp_group_affiliates', 'get_group_affiliates_sc');
function get_group_affiliates_sc(){

  $affiliates = get_group_affiliates('affiliate_group_id_10');
  echo '<pre>'; print_r($affiliates); echo '</pre>';
