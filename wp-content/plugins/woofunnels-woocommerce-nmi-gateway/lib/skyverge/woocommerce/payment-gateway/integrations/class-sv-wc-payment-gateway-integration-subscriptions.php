...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_2_1\\SV_WC_Payment_Gateway_Integration_Subscriptions' ) ) :
	...
	class SV_WC_Payment_Gateway_Integration_Subscriptions extends SV_WC_Payment_Gateway_Integration {
		...
		public function add_support() {
			...
			if ( is_callable( array( $this->get_gateway(), 'subscriptions_get_excluded_order_meta_keys' ) ) ) {
				add_filter( 'wc_payment_gateway_' . $this->get_gateway()->get_id() . '_subscriptions_order_specific_meta_keys', array(
					$this->get_gateway(),
					'subscriptions_get_excluded_order_meta_keys'
				) );
			}
		...
		public function process_renewal_payment( $amount_to_charge, $order ) {
			...
			$token = $this->get_gateway()->get_order_meta( $order_id, 'payment_token' );
			...
			if ( empty( $token ) || ! $this->get_gateway()->get_payment_tokens_handler()->user_has_token( $order->get_user_id(), $token ) ) {
				\NMI_Gateway_Woocommerce_Logger::log( "XL NMI order_id: $order_id, Token in function: " . __FUNCTION__ . " at Line: " . __LINE__ . " Token: $token, User id: {$order->get_user_id()} and user has token: " . print_r( $this->get_gateway()->get_payment_tokens_handler()->user_has_token( $order->get_user_id(), $token ), true ) );
		...
		public function update_failing_payment_method( $subscription, $renewal_order ) {
			...
			\NMI_Gateway_Woocommerce_Logger::log( "Renewal Order in update failing payment method: " . print_r( $renewal_order, true ) );
