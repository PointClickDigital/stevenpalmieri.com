...
class NMI_Gateway_Woocommerce_Credit_Card extends NMI_Gateway_Woocommerce_Base {
	...
	public function do_credit_card_transaction( $order, $response = null ) {
		...
		if ( is_null( $response ) ) {
			...
			if ( ! is_null( $response ) && $response->transaction_approved() ) {
				...
				$order->payment->tokenize       = $this->tokenization_enabled();
	...
	public function add_payment_gateway_transaction_data( $order, $response ) {
		//Add token data
		if ( $this->tokenization_enabled() && isset( $response->customer_vault_id ) ) {
	...
	public function payment_fields() {
		...
		$display_tokenization = is_checkout() && $this->tokenization_enabled() && is_user_logged_in();
		$total                = WC()->cart->total;
		$order                = null;
