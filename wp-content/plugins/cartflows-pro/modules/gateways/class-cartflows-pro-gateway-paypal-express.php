...
class Cartflows_Pro_Gateway_Paypal_Express extends Cartflows_Pro_Paypal_Gateway_helper {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_paypal_refund_request', array( $this, 'offer_refund_request_data' ), 10, 3 );
	...
	public function get_callback_url( $args ) {
		...
		$api_request_url = WC()->api_request_url( 'cartflows_ppec_paypal' );
	...
	public function modify_paypal_arguments( $data ) {
		...
		if ( true === wcf_pro()->utils->is_reference_transaction() ) {
			...
			if ( $data && isset( $data['METHOD'] ) && 'SetExpressCheckout' == $data['METHOD'] && ! isset( $data['L_BILLINGTYPE0'] ) ) {
			...
			if ( isset( $data['METHOD'] ) && 'DoExpressCheckoutPayment' == $data['METHOD'] ) {
				...
				if ( isset( $data['PAYMENTREQUEST_0_CUSTOM'] ) ) {
					...
					if ( isset( $get_custom_attrs->order_id ) ) {
						...
						try {
							...
							wcf()->logger->log( 'Details Below: ' . print_r( $e->getMessage(), true ) ); //phpcs:ignore
						}
					}
				}
			}
	...
	public function process_offer_payment( $order, $product ) {
		...
		try {
			...
			$ppec_client = wc_gateway_ppec()->client;

			$order_id = $order->get_id();

			$args = $ppec_client->get_do_reference_transaction_params(
				array(
			...
			if ( false === $order->has_shipping_address() ) {
				...
				unset( $args['SHIPTOSTREET'] );
				unset( $args['SHIPTOSTREET2'] );
				unset( $args['SHIPTOCITY'] );
				unset( $args['SHIPTOSTATE'] );
				unset( $args['SHIPTOZIP'] );
				unset( $args['SHIPTOCOUNTRY'] );
			}

			$response = $ppec_client->do_reference_transaction( $args );
