	...
	class CartFlows_Pro_Licence {
		...
		public function activation_request( $args ) {
			...
			if ( is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) != 200 ) {
				...
				$msg      = $request->get_error_message() . '<br>' . sprintf( __( 'For more information, please check this %1$sarticle%2$s.', 'cartflows-pro' ), "<a href='https://cartflows.com/docs/request-timeout-error-while-importing-the-flow-step-templates' target='_blank'>", '</a>' );
				$response = array(
					'success'    => false,
					'error_code' => 'Curl 28',
					'error'      => $msg,
				...
				return wp_json_encode( $response, true );
			}
