...
add_filter ( 'woocommerce_account_menu_items', 'pointclick_my_account_link' );
function pointclick_my_account_link( $menu_links ){
 
	// we will hook "affiliate" endpoints later
	$new = array( 'affiliates' => 'Affiliates', 'learningcenter' => 'Learning Center' );
...
add_filter( 'woocommerce_get_endpoint_url', 'pointclick_hook_endpoint', 10, 4 );
function pointclick_hook_endpoint( $url, $endpoint, $value, $permalink ){
 
	if( $endpoint === 'affiliates' ) {
 
		// ok, here is the place for your custom URL, it could be external
		$url = site_url("/affiliate-area");
	}
	if( $endpoint == 'learningcenter') {
        $url = 'https://learning.stevenpalmieri.com';
	}
    return $url;
	
}

function pointclick_add_custom_footer() {
	echo do_shortcode('[et_pb_section global_module="3900"][/et_pb_section]');
}
add_action( 'et_after_main_content', 'pointclick_add_custom_footer' );


/***** Redirect To SP.com after purchase********/

add_action( 'woocommerce_thankyou', 'pcd_redirectcustom');

function pcd_redirectcustom( $order_id ){

    $order = wc_get_order( $order_id );

    $url = 'https://stevenpalmieri.com/thank-you';
